...
func (c *Controller) handlePay(w http.ResponseWriter, r *http.Request) {
	...
	if sc != nil {
		...
		case scenario.ActionForceFailure:
			c.respondWalletError(w, scenario.Param(sc, "message", "forced failure"), scenario.ParamInt(sc, "error_code", 0))
		...
		case scenario.ActionAmbiguousError:
			...
			c.respondWalletError(w, msg, scenario.ParamInt(sc, "error_code", 0))
	...
	if errA != nil {
		c.respondWalletError(w, errA.Error(), 0)
		c.finalize(entry, started, http.StatusOK, "")
		return
	}
...
// respondWalletError повторяет форму реального отказа FreedomPay: рядом с message
// приходит числовой Code, по нему PG определяет причину отказа.
func (c *Controller) respondWalletError(w http.ResponseWriter, msg string, code int) {
	data := map[string]any{"status": "error", "message": msg}
	if code != 0 {
		data["Code"] = code
	}
